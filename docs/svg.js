...
(function () {
    ...
    // These include all the geometry, SVG path generation, and SVG DOM manipulation
    ...
    window.makeNotch = makeNotch;
    window.move = move;
    window.makeRange = makeRange;
    window.merge = merge;
    window.radians = radians;
    window.degrees = degrees;
    window.minEncode = minEncode;
    window.printPlans = printPlans;

    window.svgClear = svgClear;
    window.svgSmaller = svgSmaller;
    window.svgLarger = svgLarger;
    window.svgGrid = svgGrid;
    window.svgAppend = svgAppend;

    // svgEdit will be set up by app.js
})();
